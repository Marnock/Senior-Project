...
{
    /// <summary>
    /// This spawn system will check if enemy distance are within
    /// </summary>
    public class Kit_SpawnSystemDistanceCheck : Kit_SpawnSystemBase
    {
        ...
        public override bool CheckSpawnPosition(Kit_IngameMain main, Transform spawnPoint, Photon.Realtime.Player spawningPlayer)
        {
            ...
            int team = (int)spawningPlayer.CustomProperties["team"];
            ...
            for (int i = 0; i < col.Length; i++)
            {
                ...
                if (pb)
                {
                    if (!main.currentGameModeBehaviour.isTeamGameMode || pb.myTeam != team)
                    {
        ...
        public override bool CheckSpawnPosition(Kit_IngameMain main, Transform spawnPoint, Kit_Bot bot)
        {
            ...
            for (int i = 0; i < col.Length; i++)
            {
                ...
                if (pb)
                {
                    if (!main.currentGameModeBehaviour.isTeamGameMode || (pb.myTeam != team || Vector3.Distance(spawnPoint.position, pb.transform.position) < 1f))
                    {
                        enemyColliders.Add(col[i]);
                    }
                }
