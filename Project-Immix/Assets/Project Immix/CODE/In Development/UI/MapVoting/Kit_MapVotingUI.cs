    ...
    public class Kit_MapVotingUI : Kit_MapVotingUIBase
    {
        ...
        public override void SetupVotes(List<MapGameModeCombo> combos)
        {
            ...
            for (int i = 0; i < combos.Count; i++)
            {
                ...
                {
                    newEntry.mapName.text = main.gameInformation.allGameModes[combos[i].gameMode].traditionalMaps[combos[i].map].mapName;
                    newEntry.mapImage.sprite = main.gameInformation.allGameModes[combos[i].gameMode].traditionalMaps[combos[i].map].mapPicture;
                }
                else if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Lobby)
                {
                    newEntry.mapName.text = main.gameInformation.allGameModes[combos[i].gameMode].lobbyMaps[combos[i].map].mapName;
                    newEntry.mapImage.sprite = main.gameInformation.allGameModes[combos[i].gameMode].lobbyMaps[combos[i].map].mapPicture;
                }
